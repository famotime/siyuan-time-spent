...
思源 v3.8.3 对前端插件生命周期执行**严格串行化与超时保护契约**：

```mermaid
graph LR
    onload[onload] --> onLayoutReady[onLayoutReady]
    onLayoutReady --> onDataChanged[onDataChanged 数据变更]
    onDataChanged --> onunload[onunload 禁用/重载/卸载]
    onunload --> uninstall[uninstall 仅移除插件时]
```
...
| 方法/属性 | 适用版本 | 语义与作用 |
|---|---|---|
| `addTopBar(options)` | 全版本 | 添加顶栏按钮或自定义元素（支持桌面端、左/右侧对齐） |
...
- v3.8.3 统一支持 `execute?: (context: ICommandContext) => boolean | void`，其中 `ICommandContext` 携带触发时的 `protyle` 实例与上下文。
...
- `loadData(storageName: string): Promise<any>`：读取插件本地数据（位于工作空间 `data/storage/petal/<plugin-name>/<storageName>`）。
- `saveData(storageName: string, content: any): Promise<any>`：保存插件数据。

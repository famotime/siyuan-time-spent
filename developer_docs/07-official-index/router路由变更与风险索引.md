...
思源内核截至 v3.8.5 共有 **725+ 条注册路由**。插件调用时应根据以下三级风险模型评估：
...
- **常见模块与特征**：
  ...
  - **只读与发布端拦截 (`CheckReadonly` / `CheckPublish`)**：
    - 所有直接改写文件系统和 SQLite 的内部操作端点；只读发布端调用会立即返回 403 拒绝。
  - **工作空间边界限制 (GHSA-phmw-4rgv-r4xv)**：
    - 内核插件私有服务文件返回必须限定在当前工作空间内，越界返回 404。
  - **跨站请求伪造拦截 (GHSA-2w6q-wgc8-q743)**：
    - 外部未经授权的浏览器跨站请求一律被拦截。
